...
const Solbox = artifacts.require("./Solbox.sol");

require("chai").use(require("chai-as-promised")).should();

contract("Solbox", ([deployer, uploader]) => {
  let solbox;

  before(async () => {
    solbox = await Solbox.deployed();
  });

  describe("deployment", async () => {
    it("deploys successfully", async () => {
      const address = await solbox.address;
    ...
    it("has a name", async () => {
      const name = await solbox.name();
      assert.equal(name, "Solbox");
    });
  });
  ...
  describe("file", async () => {
    ...
    before(async () => {
      result = await solbox.uploadFile(
        fileHash,
        fileSize,
        fileType,
        fileName,
      ...
      fileCount = await solbox.fileCount();
    });
    ...
    it("upload file", async () => {
      ...
      await solbox.uploadFile(fileHash, fileSize, fileType, fileName, "", {
        from: uploader,
      }).should.be.rejected;
    });
    ...
    //check from Struct
    it("lists file", async () => {
      const file = await solbox.files(fileCount);
      assert.equal(
